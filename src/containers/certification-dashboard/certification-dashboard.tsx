...
class CertificationDashboard extends React.Component<RouteProps, IState> {
  ...
  private renderRow(collectionData: CollectionVersionSearch, index) {
    ...
    const data_cy = `${collectionData.repository.name}-${collectionData.collection_version.namespace}-${collectionData.collection_version.name}`;
    return (
      <tr key={index} className='CertificationDashboard-row' data-cy={data_cy}>
