...
function legacyStatusToPulpStatus(v1status: string): PulpStatus {
  ...
  */
  ...
  return PulpStatus[value as keyof typeof PulpStatus];
}


interface IProps {
  role: LegacyRoleDetailType;
  //detail: LegacyRoleImportDetailType;
  empty: boolean;
  task: LegacyRoleImportDetailType;
...
export class ImportConsole extends React.Component<IProps> {
    ...
    return (
      ...
      >
        ...
        <div className='message-list'>
          ...
          {task.summary_fields.task_messages.map((x, i) => {
            return this.renderMessage(x, i);
          })}

          {task.summary_fields.task_messages.length === 0 ? (
  ...
  private renderMessage(item, i) {
    return (
      <div className='message' key={i}>
        <span className={item.message_type.toLowerCase()}>{item.message_text}&nbsp;</span>
      </div>
    )
  }
  ...
  private renderTitle(selectedImport) {
    ...
    const hideCollectionName = false;
    const empty = false;
    ...
    let roleHead = (
      <>
        {role.summary_fields.namespace.name}.{role.name}
      </>
    );
    ...
    */
    ...
    const status = legacyStatusToPulpStatus(selectedImport.state);
    console.log('pulpstatus', status);

    return (
      <div>

        {/*<div className='title-container'>{roleHead}</div>*/}

        <div className='title-bar'>
          <div>
            <span className='data-title'>{t`Status:`}</span>{' '}
            <StatusIndicator type='secondary' status={status} />
          </div>
        </div>
