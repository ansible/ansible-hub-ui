...
export const Tabs = ({ tabs, params, updateParams, isDisabled, disabledTitle }: IProps) => (
  <PFTabs
    activeKey={getActiveTab({ params, tabs })}
    onSelect={(_, key) =>
      !isDisabled && updateParams(ParamHelper.setParam(params, 'tab', tabs[key].id.toLowerCase()))
    }
  >
    {tabs.map((tab, i) => (
      <Tab
        key={i}
        eventKey={i}
        title={<TabTitleText title={isDisabled ? disabledTitle : null}>{tab.name}</TabTitleText>}
...
function getActiveTab({ params, tabs }) {
  if (params.tab) {
    const i = tabs.findIndex((x) => x.id.toLowerCase() === params.tab.toLowerCase());
