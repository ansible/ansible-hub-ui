...
describe(['cloud'], 'cloud smoketest', () => {
  ...
  describe('with download', () => {
    ...
    it('can load a Collection', () => {
      ...
      cy.get('.hub-c-card-collection-container .name a', cKwargs).should('exist');

      // click on the first available collection
      cy.get('.hub-c-card-collection-container .name a', cKwargs).first().click();
    ...
    it('can load a Partners list of collections', () => {
      ...
      cy.get('.hub-c-card-ns-container .pf-c-card__footer a', cKwargs).first().click()
