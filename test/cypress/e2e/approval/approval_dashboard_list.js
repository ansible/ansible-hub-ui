...
describe('Approval Dashboard list tests for sorting, paging and filtering', () => {
  ...
  it('should sort collection.', () => {
    ...
    cy.get('[data-cy^="ApprovalRow"]:first').contains(items.at(-1).name);
    cy.get('[data-cy^="ApprovalRow"]').contains(items.at(-2).name);
    cy.get('[data-cy^="ApprovalRow"]').contains(items.at(-3).name);
  });
