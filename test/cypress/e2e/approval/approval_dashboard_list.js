...
describe('Approval Dashboard list tests for sorting, paging and filtering', () => {
  ...
  it('should contains all columns.', () => {
    ['Namespace', 'Collection', 'Version', 'Date created', 'Status'].forEach((item) => {
  ...
  it('should sort alphabetically and paging is working.', () => {
    ...
    cy.get('[data-cy="body"]').get('[aria-label="Go to next page"]:first').click();
  ...
  it('should sort collection.', () => {
    ...
    cy.get('[data-cy^="CertificationDashboard-row"]:first').contains(items[items.length - 1].name);
    cy.get('[data-cy^="CertificationDashboard-row"]').contains(items[items.length - 2].name);
    cy.get('[data-cy^="CertificationDashboard-row"]').contains(items[items.length - 3].name);
  });
  ...
  it('should filter collection.', () => {
    ...
    cy.contains('[data-cy="body"] [data-cy="compound_filter"] a', 'Collection Name').click();
    cy.get('[data-cy="sort_name"]').click();
    cy.get('[data-cy="sort_name"]').click();
    ...
    cy.get('[data-cy="body"]').contains('approval_dashboard_collection_test1').should('not.exist');
  });

  it('should filter collection and namespace together.', () => {
    cy.get('[data-cy="body"] [data-cy="compound_filter"] button:first').click();
    cy.contains('[data-cy="body"] [data-cy="compound_filter"] a', 'Collection Name').click();
    cy.get('[data-cy="body"] .toolbar input').type('approval_dashboard_collection_test0{enter}');

    cy.get('[data-cy="body"] .toolbar button:first').click();
    cy.contains('[data-cy="body"] [data-cy="compound_filter"] a', 'Namespace').click();
    cy.get('[data-cy="body"] [data-cy="compound_filter"] input').type(
  ...
  it('should set page size', () => {
    cy.get('[data-cy="body"]').get('button[aria-label="Items per page"]:first').click();
