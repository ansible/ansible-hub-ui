...
function versionCheck(version) {
  ...
  cy.get('[data-cy="PageWithTabs-AnsibleRepositoryDetail-repository-versions"]');
  cy.contains(
...
describe('Repository', () => {
  ['with remote', 'without remote'].forEach((mode) => {
    it('creates, edit and sync repository ' + mode, () => {
      ...
      if (mode == 'with remote') {
        cy.galaxykit('-i remote create', 'exampleTestRepository', 'https://www.example.com/');
      }
      cy.galaxykit('-i namespace create repo_test_namespace');

      cy.deleteNamespacesAndCollections();
      cy.galaxykit(`-i collection upload repo_test_namespace repo_test_collection`);
      ...
      cy.get('[data-cy="Page-AnsibleRepositoryEdit"] input[id="name"]').type('repo1Test');
      cy.get('[data-cy="Page-AnsibleRepositoryEdit"] input[id="description"]').type(
      ...
      cy.contains('[data-cy="Page-AnsibleRepositoryEdit"] button', 'Save').click();
      ...
      cy.contains('[data-cy="PageWithTabs-AnsibleRepositoryDetail-details"]', 'pipeline: approved');

      // try to edit it
      cy.contains('Edit').click();
      cy.get('[data-cy="Page-AnsibleRepositoryEdit"] input[id="retain_repo_versions"]')
        .clear()
        .type('5');
      ...
      cy.visit(uiPrefix + 'ansible/repositories/repo1Test/');
      cy.contains('[data-cy="PageWithTabs-AnsibleRepositoryDetail-details"]', '5');
    ...
    it('adds  collections ' + mode, () => {
      ...
      cy.get('input[aria-label="keywords"]').clear().type('repo_test_collection{enter}');

      cy.get('input[aria-label="repo_test_namespace.repo_test_collection v1.0.0"]').click();
    ...
    it('checks there are 2 versions and collection is here (' + mode + ')', () => {
      versionCheck(1);
      cy.contains(
    ...
    it('removes  collections ' + mode, () => {
      cy.login();
      cy.visit(uiPrefix + 'ansible/repositories/repo1Test/?tab=collection-versions');
